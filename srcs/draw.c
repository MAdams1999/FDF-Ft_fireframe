...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/12 13:09:04 by madams            #+#    #+#             */
/*   Updated: 2019/07/12 13:59:22 by madams           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** "colors.h" for MENU_BACKGROUND macros and BACKGROUND macros
*/
...
static void	draw_line(t_point f, t_point s, t_fdf *fdf)
{
	...
	delta.x = FT_ASS(s.x - f.x);
	delta.y = FT_ASS(s.y - f.y);
	sign.x = f.x < s.x ? 1 : -1;
	sign.y = f.y < s.y ? 1 : -1;
...
static void	draw_background(t_fdf *fdf)
{
	...
	{
		image[i] = (i % WIDTH < MENU_WIDTH) ? BACKGROUND_MENU : BACKGROUND;
		i++;
	}
}
